...
const getVideosByQueries = async(queries) => {
    ...
    const ageFilter = (contentRatingInput) => {
        let filters = [];
        if (contentRatingInput === 'Anyone') filters.push("7+", "12+", "16+", "18+");
        else {
    ...
    if (queries.genres) {
        if (queries.genres === "All") {
          ...
          logger.info(chainedQuery);
        }
      }
    
    if(queries.contentRating) {
        contentRatingInput = ageFilter(decodeURI(queries.contentRating));
        if (chainedQuery) chainedQuery = chainedQuery.find({ contentRating: { $in: contentRatingInput }});
        else chainedQuery = Video.find({ contentRating: { $in: contentRatingInput } });
        logger.info(chainedQuery);
    }
    if(queries.sortBy) {
...
module.exports = {getAllVideos, getVideosByQueries, findVideo, addVideo, changeVotes, changeViews};